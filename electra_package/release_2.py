...
from electra_package.modules_main2 import (
    analyze_backings,
    analyze_conductor_configuration,
    cluster_and_evaluate,
    extract_conductor_config,
    puntuate_and_save,
    fit_and_evaluate_conds,
    analyze_polilinia_values,
)
...
def process_vano(vano):
    ...
    try:
        ...
        logger.info(
            f"Kmeans clustering for (config, max_conds, md, max_var): {config, max_conds, md, max_var}"
        )
        ...
        if good_clust and n_conds != 3:
            ...
            )  ############## UNSCALE FITS AND CONDUCTORS

            conds = X_scaled

            conds[0, :] = scaler_x.inverse_transform(
                X_scaled[0, :].reshape(-1, 1)
            ).flatten()
            conds[1, :] = scaler_y.inverse_transform(
                X_scaled[1, :].reshape(-1, 1)
            ).flatten()
            conds[2, :] = scaler_z.inverse_transform(
                X_scaled[2, :].reshape(-1, 1)
            ).flatten()
...
def make_summary(data):
    ...
    summary = pd.DataFrame(
        columns=[
            ...
            "FLAG",
            "NUM_CONDUCTORES",
            "NUM_CONDUCTORES_FIABLE",
            "CONFIG_CONDUCTORES",
            "COMPLETITUD",
            "PUNTUACION_APRIORI",
            "PUNTUACION_APOSTERIORI",
        ]
    )
            ...
            row = pd.DataFrame(
                {
                    ...
                    "RECONSTRUCCION": vano["RECONSTRUCCION"],
                    "FLAG": vano["FLAG"],
                    "NUM_CONDUCTORES": vano["NUM_CONDUCTORES"],
                    "NUM_CONDUCTORES_FIABLE": vano["NUM_CONDUCTORES_FIABLE"],
                    "CONFIG_CONDUCTORES": vano["CONFIG_CONDUCTORES"],
                    "COMPLETITUD": vano["COMPLETITUD"],
                    "PUNTUACION_APRIORI": vano["PUNTUACION_APRIORI"]["NOTA"],
                },
                index=[i],
            )  # , "PUNTUACION_APOSTERIORI": vano["PUNTUACION_APOSTERIORI"]["NOTA"]
...
def main_pipeline(pathdata0, n_vanos):
    ...
    for i in range(len(data[:n_vanos])):
        ...
        if metrics != -1:
            ...
            plot_data(
                str(idv), cond_values, list(apoyo_values), vert_values, extremos_values
            )
            plt.show()
            bad_cases += 1
    ...
    if len(rmses) != 0:
        ...
        logger.success(
            f"Mean RMSE and mean RmaxSE: {np.array(rmses).mean().mean()}, {np.array(maxes).mean().mean()}"
        )
        logger.success(
            f"Mean correlation R Pearson and Spearman: {np.array(correlations)[:,0].mean().mean()}, {np.array(correlations)[:,1].mean().mean()}"
        )
...
def testing_pipeline(data, vanos_id):
    ...
    for id in vanos_id:
        ...
        if metrics == -1:
            plot_data(
                "", cond_values, apoyo_values, vert_values, extremos_values, "red"
            )
            plt.show()
